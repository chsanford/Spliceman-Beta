...
while ( my $line = <$fh> ) {
	...
	for(keys %matrix_length){
		...
		for(my $r = 0; $r<scalar(@{$matrix_names{$_}}); $r++){
			...
			push(@matrix_final_array, substr($matrix, 0, -4));
		}
	...
	@matrix_final_array = @matrix_final_array[@idx];
	print $fh2 join("\t",@rbp_scores_final_array[0..4])."\t".join("\t",@rbp_name_final_array[0..4])."\t".join("\t",@matrix_final_array[0..4])."\n";
